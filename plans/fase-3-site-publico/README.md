...
Última atualização: 2026-09-17 (planos 036 a 044 DONE)
...
| **Faixa de células-caminho não uniformizada com o rodapé** — a 768 px a célula ímpar não estica pela linha inteira (o `SiteFooter.astro:130-132` estica), e falta o `padding-right: 1.5rem` em `:not(:last-child)` a partir de `lg`, então o texto pode encostar na régua seguinte | **053** (verificação transversal) | Achado da revisão do 044. Não viola o §6.1, que não legisla essa faixa, e o plano autoriza "entre `sm` e `lg`, 2+". É inconsistência de padrão entre componentes do mesmo site. **Fecha quando:** o 053 uniformizar a faixa, com o rodapé como referência |
| **`src/pages/index.astro` com 181 linhas** contra o "alvo < 150" do §10.4 do PRD | **Nenhum plano ainda** — candidato a plano próprio de extração | Julgado na revisão do 044 como **alvo excedido, não invariante violado**: o §10.4 escreve "Alvo", única linha daquela tabela com hedge (as demais dizem "proibido"). Das 181 linhas, 29 são o cabeçalho §10.1 obrigatório e 27 o `<style>` com escopo. Extrair não cabia no 044, que fecha o escopo em um arquivo; e a faixa de células **reaparece no §6.2** (plano 046), então um componente extraído só da Home nasceria com a API errada. **Fecha quando:** um plano próprio extrair a faixa de células, hoje duplicada entre `SiteFooter.astro` e `index.astro` |
