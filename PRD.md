...
| RF-28 | MUST | Rotas em inglês sob `/en` com fallback para português por item | Dada uma publicação sem versão EN, quando o visitante abre `/en/publications`, então vê a entrada em português, com o elemento marcado `lang="pt-BR"` e um aviso único na página de que parte do conteúdo só existe em português (F-07), e a página nunca fica vazia | ⬜ |
...
| F-07 | Item (ou campo) sem versão em inglês na rota `/en` | Exibe o conteúdo em português, com `lang="pt-BR"` no elemento; a página mostra **um único** aviso se qualquer texto caiu no PT (sabatina fase 4, Decisão 4) | Aviso por página, do dicionário `en` (ex.: "Some content on this page is only available in Portuguese") |
