...
- [x] Identidade visual aplicada — tokens, escala tipográfica e classes de movimento no plano **036** (`fb117b6`); componentes de base (`PageHeader`, `PillButton`, `Tag`, `ExternalLink`) no **043** (`dfdc0a3`). **Conferida rota a rota no 053**, por leitura do `dist/` do triage (2026-09-23 14:38): nas 8 rotas (7 páginas + `404.html`) há exatamente um `<h1>`, a assinatura do `PageHeader` (`titulo-entrada` no `<h1>`, `regua-entrada` na régua — a Home replica a mesma marcação sem importar o componente), uma folha de estilo auto-hospedada em `/_astro/*.css` e zero `fonts.googleapis` — ver a tabela na Evidência do 053
