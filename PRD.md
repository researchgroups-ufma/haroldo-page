...
| v0.1.37 | 2026-09-16 | Desenvolvedor | **Plano 038 DONE** (`1d0d3a5`) — `src/lib/text.ts` (`toParagraphs`: texto simples do professor em parágrafos por linha em branco, sem escape e nunca para `set:html`; `padCount`: dois dígitos) e `src/lib/date.ts` (`formatDate`, §8.3: `aaaa-mm-dd` → `dd/mm/aaaa` por string, sem `Date`/`Intl`, texto livre inalterado). Teste de determinismo com `TZ=America/Fortaleza`, confirmado discriminante também em máquina UTC. Não fecha item do §12 (pré-requisito). Revisão reprovou um ciclo: o `afterEach` gravava a string `"undefined"` em `process.env.TZ` em vez de restaurar a ausência — corrigido com `delete` e provado por canário. Suíte em 167 testes, cobertura 100% statements/lines e 93,75% branches; CI `qualidade` (run 35156351826) e Workers Builds `success` sobre `1d0d3a5` |
| v0.1.38 | 2026-09-16 | Desenvolvedor | **Plano 039 DONE** (`5705e73`) — `src/lib/published.ts` (`filterPublished`, RN-01; `requireSingleton`, erro que nomeia coleção e contagem) e `src/lib/research.ts` (ordenação de linhas por `ordem` e título pt-BR, de projetos por status, contagem de projetos em andamento por linha, e `relatedLineAnchor`, que não gera âncora para linha não publicada — RN-01). Sem `astro:content` nos módulos; compatibilidade com `CollectionEntry` real provada por arquivo temporário no `astro check`. Não fecha item do §12 (pré-requisito). Revisão reprovou um ciclo por dois testes que não discriminavam a regra ("Óptica"/"Ondas" igual com e sem pt-BR; desempate com `ordem` igual sem teste), corrigidos com canário. Suíte em 185 testes, cobertura 100% statements/lines e 97,82% branches; CI `qualidade` (run 35157907737) e Workers Builds `success` sobre `5705e73` |
| v0.1.39 | 2026-09-16 | Desenvolvedor | **Plano 040 DONE** (`6bf5aa4`) — `src/lib/publications.ts`: `groupByYear` (ano decrescente, RF-25), `compareWithinYear` como único lugar da regra dentro do ano (título alfabético pt-BR, provisória até existir campo de data de cadastro — RN-02 mantida, Q-RN02 opção (c)), `isProfessorAuthor` (igualdade exata após `trim`, espaços, NFD sem diacríticos e minúsculas), `doiUrl` e `arxivUrl` aceitando prefixos e espaços. Não fecha item do §12 (pré-requisito do 050). Revisão reprovou dois ciclos: testes que não discriminavam a regra (acento, `trim`, prefixo `http` do arXiv, `trim` de DOI/arXiv, fixture de destaque fora de `data`) e uma nota da Evidência que afirmava sem prova a origem de diacríticos combinantes no regex — o estado final em bytes é o escape ASCII, e a origem ficou registrada como hipótese. Suíte em 211 testes, cobertura 100% statements/lines e 98% branches; CI `qualidade` (run 35160493418) e Workers Builds `success` sobre `6bf5aa4` |
| v0.1.40 | 2026-09-17 | Desenvolvedor | **Plano 041 DONE** (`a2e28b7`) — `src/lib/courses.ts`: `courseSlug` deriva a URL do nome do arquivo com `slugify` (`2026.2-relatividade-geral.md` → `2026-2-relatividade-geral`), e não do `entry.id` do Astro, que remove o ponto (id real observado no build: `20262-relatividade-geral`); `buildCourseSlugs` reprova slug duplicado nomeando os dois caminhos, para o `getStaticPaths` do 048 falhar em vez de sobrescrever página; `splitCourses` separa por `status` sem lógica de data (RN-03), semestre decrescente numérico pt-BR e empate por nome; `countCourseItems`; `groupScriptsByLesson` alinhado por índice (RN-04), número de aula repetido vai para a primeira, órfãos em `general` — nenhum script some (F-13); `presentSections` na ordem fixa do "Nesta página", com Aulas sempre presente (F-06). Não fecha item do §12 (pré-requisito do 044, 047, 048 e 049). Revisão reprovou dois ciclos: testes que não discriminavam a regra (erro de `courseSlug` aceitava qualquer exceção; grupo `current` de `splitCourses` sem verificação e depois sem ordem testada), blocos de canário sem comando rotulado e um bloco de `astro build` redigitado com horário alterado — refeito a partir das saídas capturadas e comparado byte a byte. Suíte em 236 testes, cobertura 100% statements/lines e 98,88% branches; CI `qualidade` (run 35247024594) e Workers Builds `success` sobre `a2e28b7` |
| v0.1.41 | 2026-09-17 | Desenvolvedor | **Plano 042 DONE** (`db96df3`) — primeiro plano com página visível: `src/layouts/BaseLayout.astro` (`lang="pt-BR"`, `<title>` pelo dicionário com a Home passando `title={undefined}`, "Pular para o conteúdo" como primeiro focável, `<main id="conteudo">`, perfil por `requireSingleton`; canonical, OG e favicon continuam na fase 5), `src/components/SiteHeader.astro` (nome de `siteConfig.shortTitle`, `aria-current` e sublinhado na rota ativa, menu do celular por grade `1fr`/`0fr` em 150 ms com `aria-expanded` e `Esc`, aberto e sem botão quando o JS não roda — RNF-02; espaço vazio reservado para o seletor da fase 4) e `src/components/SiteFooter.astro` (quatro células, grade em três faixas, célula de perfis some sem link e a ímpar estica na linha inteira). **Fecha o item "Layout base, cabeçalho, rodapé e navegação" do §12 da fase 3.** Verificação no navegador a 360, 768 e 1440 px sem rolagem horizontal, com três defeitos achados ali e corrigidos antes da revisão (menu fechado ainda focável, buraco no rodapé a 768, recuo fora da margem da grade); a revisão reprovou um ciclo (`↗` lido por leitor de tela, `title` opcional, trechos da Evidência não gerados por comando). O teste de **Tab real** foi dispensado pelo stakeholder e passa ao 053, com a dívida registrada no README da fase. Suíte em 236 testes, `build:pipeline` verde, `astro check` 0/0/0; CI `qualidade` (run 35254730157) e Workers Builds `success` sobre `db96df3` |
| v0.1.42 | 2026-09-17 | Desenvolvedor | **Plano 043 DONE** (`dfdc0a3`) — os quatro componentes de base que todas as rotas dos planos 044–051 reúsam: `src/components/PageHeader.astro` (rubrica `text-rotulo`, `<h1 class="text-display-1 titulo-entrada">`, slot `aside` opcional em grade `7/5` a partir de `lg` e empilhado abaixo, régua forte `aria-hidden` de ponta a ponta — sem declarar animação própria, só as classes `regua-entrada`/`titulo-entrada` do 036), `PillButton.astro` (`<a>` de 48 px, disco de 32 px com `›` oculto, variantes `outline` e `solid`, transição de fundo em 120 ms), `Tag.astro` (variantes `strong`/`neutral`/`solid`, `dot` de 6 px, `dashed` sempre em `--tracejado` por reserva do §2 da identidade, `normalCase` para as áreas de atuação do §6.2) e `ExternalLink.astro` (`target="_blank"`, `rel="noopener noreferrer"`, `↗` oculto e "(abre em nova aba)" pelo dicionário). **Fecha, com o 036, o item "Identidade visual aplicada" do §12** — em 2026-09-17, quando este plano fechou, nenhuma rota ainda usava os componentes; o 053 conferiu rota a rota sobre o build final e confirmou a assinatura do `PageHeader` (ou markup equivalente, na Home) nas 8 rotas. Verificação no navegador a 360 e 1440 px sem rolagem horizontal, com a régua a 1440 px de ponta a ponta e a grade `7/5` observada; a armadilha "animação que começa invisível esconde texto" foi descartada desligando em runtime o único bloco `@media (prefers-reduced-motion: no-preference)` e observando tudo visível e sem animação — e o `global.css:174-182` ainda zera duração de animação e transição sob `reduce`. A **emulação de `reduce` pelo DevTools e a navegação por Tab não foram feitas** (a extensão do Chrome não as permite nesta máquina) e passam ao 053, junto com a dívida equivalente do 042. A revisão reprovou um ciclo, com dois bloqueantes textuais: a Evidência colada descrevia os arquivos de antes do `prettier --write` (o `prettier-plugin-tailwindcss` reordenou classes, sem efeito de cascata) e três comentários citavam seção inexistente na fonte nomeada (`§8.2 da identidade`, `§5.5 do PRD`, e o `§5.4` para uma cor que só o `§2` reserva). Suíte em 236 testes, cobertura 100% statements/lines e 98,88% branches; `build:pipeline` verde e `astro check` 0/0/0 em duas execuções autoritativas idênticas |
| v0.1.43 | 2026-09-17 | Desenvolvedor | **Plano 044 DONE** (`4e48ce5`) — a Home, primeira rota com conteúdo real do professor: `<h1>` = `perfil.nome` (§8 da identidade), coluna direita com `cargo — departamento` (o trecho do departamento só quando o campo existe, que é `.optional()` no schema), `instituicao`, `resumo_home` em parágrafos por `toParagraphs` e o `PillButton` para `/pesquisa/`; régua `regua-entrada` de ponta a ponta; e a faixa de **três células-caminho** (Pesquisa, Ensino, Publicações), cada uma um `<a>` inteiro, com numeral de dois dígitos por `padCount`. **Fecha o item "Home (RF-20)" do §12.** As contagens são só de publicados (**RN-01**): 02 linhas, 01 disciplina atual (por `splitCourses(...).current`), 05 publicações — e a regra foi provada **por falsificação**, não por afirmação: o canário que remove `filterPublished` leva a contagem a **06**, trazendo para o site o arquivo com `publicado: false`. Acessibilidade das células: os três blocos visuais ficam `aria-hidden` e só um `<span class="sr-only">` é exposto, com o numeral **real** (2, 1, 5) e não o `padCount`. Sem `perfil.foto`, faixa de três colunas e **zero `<img>`** (F-08). Verificação no navegador a 360, 768 e 1440 px sem rolagem horizontal, com a grade indo a 1, 2 e 3 colunas e a divisão 7/5 confirmada no bloco superior. **Revisão aprovada sem bloqueante**, com três achados registrados e nenhum resolvido aqui: (1) `pt.home.noneYet` ("nenhuma publicada ainda") é **semanticamente errado para a célula de Ensino**, porque zero disciplinas *atuais* não significa nenhuma publicada — o defeito está no **§6.1 da identidade**, que prescreve esse texto para toda contagem zero, e não no código; pende de decisão do dono do produto; (2) a faixa de células não estica a célula ímpar a 768 px nem tem o `padding-right` que o rodapé tem a partir de `lg` — vai para o 053; (3) `index.astro` ficou com **181 linhas** contra o "alvo < 150" do §10.4, julgado como alvo excedido e não invariante violado (29 linhas são o cabeçalho §10.1 obrigatório e 27 o `<style>`), com a extração virando **dívida para plano próprio**, já que o 044 fecha o escopo em um arquivo e a faixa de células reaparece no §6.2. Suíte em 236 testes, cobertura 100% statements/lines e 98,88% branches; `astro check` 42 arquivos 0/0/0 |
| v0.1.44 | 2026-09-18 | Desenvolvedor | **Plano 045 DONE** (`369637a`) — a rota **Sobre**, segunda com conteúdo real do professor: biografia por `toParagraphs` no slot `aside` do `PageHeader`; **formação em grade de 3 colunas** (`ano` · `grau — curso` · `instituicao`) com régua fina entre linhas e forte na primeira, empilhando abaixo de `sm`, e as **5 entradas na ordem do arquivo** — nenhum `sort`, porque a ordem é a do professor; faixa de células com as 4 áreas como `Tag variant="neutral" normalCase` (§5.4), contato em `mailto:` sublinhado e os perfis acadêmicos na ordem fixa `lattes, orcid, scholar, arxiv, researchgate, github, institucional` com `cv_url` por último, todos por `ExternalLink` (§5.5) — zero `target="_blank"` cru no arquivo. **Fecha o item "Sobre (RF-21)" do §12**, levando a fase 3 a 4/12. O RF-21 ("apenas os campos preenchidos") foi provado onde importa: cada guarda envolve **rubrica + conteúdo**, não só o conteúdo, então sem `foto` e sem `cv_url` a página sai com **zero `<img>`**, nenhum "Currículo em PDF" e exatamente **quatro `<h2>`**. Verificação no navegador a 360, 768 e 1440 px sem rolagem horizontal (345/345, 753/753, 1425/1425), com a formação em 1 coluna a 360 e 3 a partir de 768, e a célula ímpar final esticando a linha inteira a 768 — a correção que a revisão do 044 pediu, aplicada aqui mas **ainda pendente no `index.astro`**. **Revisão aprovada sem bloqueante.** Duas lições de **instrumento**, não de código, vieram deste plano e mudam os canários dos planos 046–051: (1) o `dist/**/index.html` é minificado em **uma única linha**, e `grep -c` conta linhas, não ocorrências — só pode devolver 0 ou 1 mesmo com rótulo órfão de verdade (medido: `grep -c` → 1, `grep -o ... | wc -l` → 3); (2) o canário de rótulo órfão sobre a **página inteira** é inatingível por construção, porque o `BaseLayout.astro:53` busca o perfil por conta própria e o rodapé renderiza "Perfis acadêmicos" em toda rota — o teste certo escopa ao `<main>`, onde o resultado é 0. O executor **reportou a divergência em vez de contorná-la**, e o revisor a reproduziu. O foco por tecla **Tab** segue não observado (a extensão do Chrome não move o foco nesta máquina) e acumula no 053, agora por quatro planos seguidos; a regra `:focus-visible { outline: 2px solid var(--color-tinta) }` foi confirmada por CSSOM no CSS entregue, o que não substitui ver o anel. `sobre.astro` ficou com **217 linhas** contra o "alvo < 150" do §10.4, julgado como alvo excedido e não invariante violado, com a extração da faixa de células — hoje em **três cópias** (`SiteFooter.astro`, `index.astro`, `sobre.astro`) — virando dívida de plano próprio. Suíte em 236 testes, cobertura 100% statements/lines e 98,88% branches; `astro check` 43 arquivos 0/0/0; CI `qualidade` (run 35348534672) e Workers Builds `success` sobre `369637a` |
| v0.1.45 | 2026-09-18 | Desenvolvedor | **Plano 046 DONE** (`97e006d`) — a rota **Pesquisa**: `src/pages/pesquisa.astro` (134 linhas) e `src/components/ProjectCard.astro` (94), **primeiro componente extraído de uma rota** nesta fase e as duas primeiras páginas abaixo do "alvo < 150" do §10.4 (o 044 ficou em 181 e o 045 em 217). Linhas de pesquisa na ordem de `ordem` por `sortResearchLines`, cada uma com numeral de posição `padCount(i+1)` marcado `aria-hidden`, `id` de âncora, `resumo` e `corpo` em parágrafos por `toParagraphs`, `imagem` só se houver (zero `<img>` hoje, F-08), e a tag "N projetos em andamento" por `countActiveProjectsByLine` — sai no **singular** com N=1 e some com zero. Projetos por `sortProjects` (em andamento antes de concluídos) em grade `repeat(auto-fill, minmax(17rem, 1fr))`, com status como `Tag` forte+marcador ou neutra tracejada (§5.4), período **sem traço quando não há `fim`** ("2025.1" contra "2023.1–2025.2"), financiador entre réguas, e link para a âncora da linha relacionada. **Fecha o item "Pesquisa (RF-22)" do §12**, levando a fase 3 a 5/12. **Três canários, todos por falsificação:** `new Set()` vazio em `relatedLineAnchor` faz a âncora `#sombras-de-buracos-negros` sumir (RN-01); `projects = []` faz a seção "Projetos" sumir inteira, com a rubrica virando "2 linhas · 0 projetos" como prova colateral; e a contagem de parágrafos por seção (2 na primeira linha, 1 na segunda) prova o `corpo` renderizado. O segundo foi exigido na orquestração porque o critério estava provado só por leitura de código. Verificação no navegador a 360/768/1440 px sem rolagem horizontal (345/345, 753/753, 1425/1425), cards empilhando a 360, e **clique real na âncora** levando a seção alvo ao topo do viewport. **Revisão reprovou o ciclo 1** com dois defeitos textuais — comentário citando `§6.3` para a regra da régua, que está no **§5.5** (mesma classe que reprovou o 043), e a Evidência afirmando "nenhuma divergência de conteúdo" quando `content/linhas-pesquisa/relatividade-geral-...md:6` **tem** `corpo`, deixando esse elemento do critério 1 sem prova. Corrigidos, com suíte autoritativa **rodada de novo sobre os bytes corrigidos** (ciclo 2) e revisão aprovada. Registrado um **incidente de colisão de builds causado pela orquestração**: dois `build:pipeline` simultâneos no mesmo working tree, proibido pelo README da fase, com o segundo falhando em `Tina Dev server is already in use ... port 9000`; nenhum artefato contaminado — a falha morre no passo do Tina, antes do `astro build` — e a sequência com horários está na Evidência. Duas observações sem ação aqui: a grade `auto-fill` deixa ~57% da largura vazia a 1440 px com dois cards, o que é **conformidade literal com o §6.3** da identidade (emenda para `auto-fit` é decisão do dono do produto), e `pt.research.eyebrow` segue sem uso desde o 037. O foco por tecla **Tab** continua não observado e acumula no 053, agora em cinco planos. Suíte em 236 testes, cobertura 100% statements/lines e 98,88% branches; `astro check` 45 arquivos 0/0/0; CI `qualidade` e Workers Builds `success` sobre `97e006d` |
| v0.1.46 | 2026-09-18 | Desenvolvedor | **Plano 047 DONE** (`4d90d92`) — a rota **Ensino**, `src/pages/ensino.astro` (155 linhas): os dois grupos que o RF-23 exige, "Atuais" antes de "Anteriores", **sempre rotulados**, com `splitCourses(filterPublished(...))` separando por `status` e ordenando por `semestre` decrescente. Atuais em células de 2 colunas a partir de `sm`, cada uma um `<a>` inteiro: `codigo · semestre` em pequeno, `nome` em `display-2`, primeiro parágrafo da `descricao` por `toParagraphs`, rodapé só com as contagens **não nulas** de `countCourseItems` ("5 aulas · 2 listas · 1 script") e "Abrir ›" — **sem numeral**, porque disciplina atual não é sequência (§8). Anteriores em linhas `codigo · nome · descricao · semestre ›` (§5.5), empilhando abaixo de `sm`. Links pelo slug do 041 com barra final. **Fecha o item "Ensino (RF-23)" do §12**, levando a fase 3 a 6/12. A **RN-03** fica explícita no código: a transição de atual para anterior é o campo `status`, sem lógica de data. O estado vazio foi provado **por falsificação** — com `current = []` a caixa tracejada "Nenhuma disciplina neste semestre." aparece 1× (contada com `grep -o ... | wc -l`, nunca `grep -c`, pela lição do 045) e o `<h2>` "Atuais" **continua na página**, que é o que distingue o RF-23 de uma implementação ingênua onde o grupo vazio some. Verificação no navegador a 360/768/1440 px sem rolagem horizontal (345/345, 753/753, 1425/1425), com `elementFromPoint` nos quatro cantos e no centro de cada célula e de cada linha provando que o `<a>` cobre o alvo inteiro nos **seis** casos, e `aria-current="page"` em "Ensino" — feita no **Vivaldi**, porque o Chrome deixou de existir nesta máquina, pela mesma técnica de `<iframe>` dos planos 042–046. **Revisão aprovada sem item obrigatório**, com seis observações endereçadas ao 053; duas valem registro: o ramo `noPrevious` **não** teve canário (o passo 4 do plano pedia um só, e o critério foi redigido no singular — o critério não foi afrouxado, o resultado é que coube nele), e `codigo` ausente deixa a primeira coluna da linha anterior vazia, conferido no navegador a 1440 px ("Mecânica Clássica" começa em x=168 em vez de x=56). O segundo **não é defeito**: é tabela de quatro colunas com célula vazia, que alinha os nomes quando houver vários registros, e o §6.4 não legisla o caso. O foco por tecla **Tab** segue não observado e acumula no 053, agora em seis planos. Suíte em 236 testes, cobertura 100% statements/lines e 98,88% branches; `astro check` 46 arquivos 0/0/0; CI `qualidade` (run 35394287388) e Workers Builds `success` sobre `4d90d92` |
| v0.1.47 | 2026-09-18 | Desenvolvedor | **Plano 048 DONE** (`5eaf30e`) — a **página de disciplina**, primeira rota **dinâmica** da fase: `src/pages/ensino/[slug].astro` (146 linhas) com `getStaticPaths` gerando uma página por disciplina publicada (RN-01/RF-10), mais `LessonList.astro` (88) e `CourseResources.astro` (148), extraídos para os três ficarem abaixo do alvo do §10.4. Trilha `Ensino / codigo` — ou `nome` quando não há código, confirmado nas duas rotas; cabeçalho com tags `codigo` forte, `semestre` e status neutros, `descricao` em parágrafos e o índice "Nesta página" montado por `presentSections` com a contagem por seção; ementa só quando há; **Aulas sempre**, com "Nenhuma aula publicada ainda." em caixa tracejada (F-06); listas com "Entrega 07/09/2026"; materiais com tipo legível; bibliografia em `<ol>`; links como tag. Datas em `dd/mm/aaaa` por formatação de string, sem `Date`. Scripts ficam para o 049 — só o `general` de `groupScriptsByLesson` é calculado e **nenhuma** seção "Scripts da disciplina" é emitida. **Fecha o item "Página de disciplina (RF-24)" do §12**, levando a fase 3 a 7/12. **Primeiro plano da fase a exigir dois ciclos de revisão.** Obrigatório 1: o cabeçalho de `CourseResources.astro` citava **F-08** para a regra "seção vazia some", mas o F-08 do §14 é *imagem ausente* e a regra está no §6.5 da identidade — **terceira ocorrência da mesma classe de defeito na fase**, depois do 043 (§6.3 por §5.5) e do 046. Obrigatório 2: o critério "rascunho não gera página" estava `[x]` por **leitura** do `filterPublished`, sem caminho negativo exercitado — a orquestração **autorizou um canário que toca `content/`**, fora da lista de "Arquivos afetados" e por isso vedado ao executor sozinho: com `publicado: false` em Mecânica Clássica o build saiu com **uma** página de disciplina e o diretório da outra sumiu; revertido por `git checkout --` (seguro porque o arquivo **está** commitado — o oposto dos três fontes novos, onde apagaria o trabalho) e rebuildado com as duas de volta. Verificação no navegador nas duas rotas a 360/768/1440 px sem rolagem horizontal (345/345, 753/753, 1425/1425 e 1440/1440), com as seis âncoras de "Nesta página" **navegando de verdade** (clique real levando a seção ao topo do viewport), estado vazio com `rgb(154,150,145)` = `--color-tracejado` medido por CSSOM, ordem RN-04 `Aula 1…Aula 5` no artefato, `aria-current="page"` único, e o **fluxo D do §8.1 fechando em exatamente quatro toques**, que é a meta do PRD. A divergência do passo 3 — o plano previa 11 `target="_blank"` e o comando deu **13** — foi **reportada, não contornada**: são 11 dentro do `<main>` e 2 do rodapé (`perfil.links`), presente em toda rota; reproduzida independentemente pelo orquestrador e pelo revisor. Registrada uma falha **da orquestração**: a seção da verificação no navegador foi escrita mas nunca inserida no plano, e o despacho da revisão afirmou que estava lá — o revisor abriu o arquivo em vez de acreditar, reprovou o ciclo 2, e depois **retirou por conta própria** a causa que havia atribuído ao script do executor, que era dedução e não medição. Duas dívidas novas no README da fase: a tag-link duplicando a composição de `Tag.astro` (legítima — `Tag` emite `<span>` e link externo tem de sair por `ExternalLink`), e o risco, lido no script e **não** observado, de o gerador de Evidência apagar seções do orquestrador. O foco por **Tab** segue não observado e acumula no 053, agora em sete planos. Suíte em 236 testes, cobertura 100% statements/lines e 98,88% branches; `astro check` 49 arquivos 0/0/0; CI `qualidade` (run 35402865607) e Workers Builds `success` sobre `5eaf30e` |
| v0.1.48 | 2026-09-18 | Desenvolvedor | **Plano 054 DONE** (`23a003d`) — portão de qualidade, **sem fechar item do §12** (como o 023 e o 052). `src/lib/prd-citations.ts` (133 linhas) e `tests/lib/citacoes-do-prd.test.ts` (206, 22 testes) conferem que **cada uma das 236 citações de identificador do PRD em `src/**` existe** na fonte declarada — `PRD.md` para `RF/RN/RNF/F/M/D/NG/A/R`, `PRD.md` **ou** `docs/identidade-visual.md` para `§N.N` —, com canário de `§99.9` provando a reprovação nomeando arquivo e linha. **O resultado negativo é a parte importante:** a camada de *pertinência* que o plano propunha (limiar sobre sobreposição de vocabulário entre o comentário e a cláusula) foi **falsificada por medição** e não entregue — 32 das 236 citações legítimas pontuam **zero**, entre elas `src/lib/published.ts:37`, que é um `// RN-01` **sozinho na linha**, e a cláusula de RN-01 (`PRD.md:277`) não contém a palavra "rascunho" com que é corretamente citada seis vezes; como a citação errada conhecida também pontua zero, nenhum limiar escalar separa os dois lados. O plano registra isso como hipótese refutada, **não** como critério afrouxado, e a revisão validou explicitamente a distinção (um critério afrouxado é o que *esta* implementação não atingiu; um critério partido é o que *nenhuma* implementação atinge). **Dois achados de documentação:** (1) o próprio arquivo criado para caçar citação trocada nasceu com uma — citava `§10.4` ("Convenções Gerais") para a regra que está no **`§10.3`** ("Comentários no Código") —, **quarta ocorrência** da série 043/046/048, detectada pela revisão; (2) **`F-08` é ambíguo na documentação**: `PRD.md:298` o define como *"Imagem ausente"*, mas `docs/identidade-visual.md:32` o amarra a *"Campo vazio não deixa rastro"* em cláusula normativa, ao lado de RF-21 e F-05 — o código convive com os dois sentidos (`ProjectCard.astro:10` e `sobre.astro:10` amplo, `index.astro:87` estrito). Essa ambiguidade **está na origem do defeito apontado no 048** e a emenda, que toca os dois documentos, é decisão do dono do produto; enquanto não houver, teste de pertinência é inescrevível por falta de verdade contra a qual calibrar. Três ciclos de revisão, e **as reprovações do segundo e do terceiro foram todas de texto do orquestrador** — seção de navegador jamais inserida, afirmação não medida sobre as 32 citações, fonte errada atribuída à frase do F-08 e números de linha desatualizados —, nenhuma do executor. Suíte em 258 testes (eram 267 antes da remoção das funções da camada refutada), cobertura 100% statements/lines e 99,12% branches; `astro check` 52 arquivos 0/0/0; CI `qualidade` (run 35410116740) e Workers Builds `success` sobre `23a003d` |
| v0.1.50 | 2026-09-22 | Desenvolvedor | **Plano 050 DONE** (`5b97655`): a página **Publicações**. `src/pages/publicacoes.astro` usa só `filterPublished` e `groupByYear`, sem reordenar nada; dentro do ano vale a regra provisória do 040 até existir campo de data de cadastro (Q-RN02, opção (c)). `src/components/PublicationItem.astro` monta cada item: tags "Destaque" e tipo, autores com o professor em `--tinta` e `<strong>`, veículo em itálico, links DOI/arXiv/PDF só quando preenchidos (F-05) e `<details>` fechado só com resumo ou palavras-chave. A RN-01 foi provada por canário: sem `filterPublished`, a rubrica vai a "6 itens". Um canário autorizado em `content/` exercitou o link arXiv e o item sem `<details>`, e foi revertido com `git diff` vazio. Três ciclos: a revisão reprovou o 1º por citar o §6.6 onde a régua forte é do §5.5 e por três lacunas de prova; a medição no navegador achou o ano estourando a coluna de `8rem` (a 0,6 px dos itens a 1440 px) e depois `auto` desalinhando os blocos, e a correção final é `11rem`. **Fecha o item "Publicações agrupadas por ano (RF-25)" do §12** (9/12). |
| v0.1.51 | 2026-09-22 | Desenvolvedor | **Plano 051 DONE** (`8c6821c`): a **página 404**. `src/pages/404.astro` segue o §6.7 da identidade: `PageHeader` com a rubrica "Erro 404", o texto do dicionário, a pílula sólida "Voltar ao início" (§5.3) e a lista "Todas as páginas" com as cinco rotas de `NAV_ITEMS`, sem CV (Decisão 3) e sem exibir o caminho digitado. O build gera `dist/404.html`, não `dist/404/index.html`. **Quita a dívida da fase 2** "`not_found_handling` ainda não provado" sem tocar no `wrangler.toml`. No `wrangler dev` local, a rota inexistente dá `404` com a página e `/sobre/` dá `200`; o canário com o arquivo renomeado devolve `404` com corpo de 0 caracteres. Em produção, antes do push, `404` com `Content-Length: 0`; depois, com CI `qualidade` e Workers Builds `success` e a versão `19c88528-bff3-4ec4-913b-213278224d0f`, o corpo tem o mesmo SHA-256 do `dist/404.html`. Navegador a 360/768/1440 px sem rolagem horizontal; o Tab segue como dívida do 053. Dois ciclos de revisão; o 1º reprovou só por documentação (o único `<script>` da página é o do menu do `SiteHeader`, byte a byte igual ao das outras rotas). **Fecha o item "Página 404 (RF-27)" do §12** (10/12). |
| v0.1.52 | 2026-09-23 | Desenvolvedor | **Plano 052 DONE** (`9af755e`): testes de integração sobre o `dist/`. `tests/dist/site-gerado.test.ts` (9 testes, 7 asserções do plano), `vitest.dist.config.ts` e o script `test:dist`, excluído da suíte padrão porque o `test:coverage` roda antes do build; o passo novo do CI vem depois do `build:pipeline`, que não mudou. O teste reprova se faltar rota ou `404.html`, se uma disciplina publicada não tiver página ou sobrar pasta sem disciplina (RF-10), se o título de um rascunho aparecer cru ou escapado em qualquer HTML (RN-01), se uma página não tiver exatamente um `<h1>` e `lang="pt-BR"`, se houver fonte do Google, se o JS de uma rota (`<script src>` com imports estáticos seguidos recursivamente e sem repetição, mais os inline) passar de 50 KB gzip, ou se aparecer `react-dom` (RNF-02). `dist/admin/` fica fora. Tabela no CI (run 35892107311): 259 bytes por rota, 606 na página de Relatividade Geral. Dez canários sobre o `dist/`. O `.gitignore` foi ancorado em `/dist/`, com autorização da orquestração. Localmente o Vitest 4.1.11 detecta o ambiente de agente e descarta o `console.log` de teste aprovado; a tabela exige `--reporter=verbose` fora do CI. Três ciclos de revisão. **Sem item do §12** (continua 10/12). |
| v0.1.49 | 2026-09-21 | Desenvolvedor | **Plano 049 DONE** (`1a028ce`): o **painel de script** da página de disciplina. `src/lib/code-theme.ts` define `monochromeTheme`, um tema Shiki com exatamente três cores de primeiro plano — `#111112` como padrão, `#5A5754` para palavra-chave e `#6E6A66` para comentário e string —, sem negrito nem itálico, e `shikiLanguage`, tipado pelo enum de `linguagem` do schema. `ScriptPanel.astro` (132 linhas) usa o `<Code>` de `astro:components`, então o destaque é gerado no build, não há JS de destaque e **nenhuma dependência foi acrescentada**. O botão "Copiar código" nasce `hidden` e só aparece se a Clipboard API existir; ele copia o `textContent` de `pre > code`, igual byte a byte ao `codigo` (323 B), e mostra "Copiado" no botão e na região `aria-live` durante 2 s. `LessonList.astro` põe os painéis sob a aula cujo número casa, e `[slug].astro` ganha a seção "Scripts da disciplina" só quando há órfãos (F-13). **Fecha o item "Scripts da disciplina…" (RF-37, F-13) do §12**, e a fase 3 passa a 8/12. O F-13 foi provado **por canário em `content/`**: com `aula: 99`, a seção apareceu depois do fim da lista de aulas, com o painel dentro dela, e o arquivo foi revertido por `git checkout --` com a reversão provada. Um segundo canário, removendo `url`, fez "Abrir arquivo" sumir. O script do copiar sai **inlinado** no HTML, sem `dist/_astro/*.js`, com 344 B em gzip; o limite de 50 KB do RNF-02 é conferido no 052. **Dois ciclos de revisão.** O ciclo 1 reprovou por três obrigatórios: (1) a região `aria-live` recebia "Copiado" e nunca era limpa, contra o "por 2 s" do §5.6; (2) a classe `font-codigo` no contêiner não chegava ao código, porque o preflight do Tailwind declara `font-family` diretamente em `pre` e `code` com a pilha monoespaçada padrão dele — corrigido com `[&_pre]:font-codigo [&_code]:font-codigo`, que fica na camada `utilities` e tem especificidade maior; (3) um `?? []` em `LessonList` que só servia ao canário. Removê-lo invalidou o canário original, que foi refeito pelo caminho de conteúdo. Verificação no navegador a 360/768/1440 px sem rolagem horizontal (345/345, 753/753, 1425/1425). A 360 px o `<pre>` rola por dentro (431 > 255), a fonte computada é a do §3 em 14 px, colar de verdade devolveu os 323 caracteres com a indentação intacta, "Copiado" durou 2063 ms e voltou a ser anunciado no clique seguinte, o Enter real acionou o botão, e sem JS o botão fica oculto com o código visível. A navegação por Tab e o estado `reduce` seguem não observados e acumulam no 053. `[slug].astro` passou a 163 linhas e entrou na dívida das páginas acima do alvo do §10.4. Suíte em 263 testes, cobertura 100% statements/lines e 99,12% branches; `astro check` 55 arquivos 0/0/0; CI `qualidade` (run 35640972806) e Workers Builds `success` sobre `1a028ce` |
| v0.1.53 | 2026-09-23 | Desenvolvedor | **Plano 053 — verificação transversal e fechamento da fase 3** (execução registrada; `Status:` fica `TODO` até a promoção pelo orquestrador). Sobre o mesmo build (SHA `c1ca1c1`), as 8 rotas × 3 larguras (360/768/1440 px) deram `scrollWidth = clientWidth` nas 24 linhas, sem elemento cortado (RF-26); único achado, não-bloqueante: a 360 px o código do painel de script rola dentro do próprio `<pre>`, não na página. O 404 do `wrangler dev` bateu SHA-256 com `dist/404.html`. O movimento (RF-32) foi observado pelo orquestrador em `no-preference` (duas animações) e sem CSS (texto sempre visível); o estado `reduce` e a navegação completa por teclado (RNF-15) foram **verificados à mão pelo stakeholder em 2026-09-23** e declarados ao orquestrador, não observados por ele — fecha as duas dívidas acumuladas desde o 042. A RN-02 do §5.3 ganhou a nota de "parcial" da Q-RN02 (opção c). Corrigidos, sem mudar lógica de teste, os comentários das dívidas 7(b) (`tests/content/paridade-schema.test.ts`) e 7(c) (`tests/content/conteudo-valido.test.ts`). **Suíte medida:** `npm run test:coverage` — 263 testes em 16 arquivos, cobertura 100% statements/lines/funções e 99,12% branches; `npm run test:dist` — 9 testes em 1 arquivo; total **272 testes em 17 arquivos**. `npm run lint` e `npm run format:check` verdes. A faixa de células sem `padding-right` uniformizado com o rodapé foi **redirecionada** (decisão do stakeholder) para um plano próprio de extração, junto da mesma faixa duplicada em três cópias — este plano não toca `src/`. **§12 da fase 3 passa a 12/12 e a fase a 🟢 concluída, no texto** — a promoção do plano (commit, CI e Workers Builds no commit de fechamento) fica para o orquestrador. Ver "O que a fase 3 empurra adiante, e as dívidas que ela criou" no README da fase para o destino de cada pendência que sobra sem fase (F-08 com duas definições, `pt.home.noneYet`, páginas acima de 150 linhas, Tailwind varrendo `plans/`/`docs/`, grade `auto-fill`, `pt.research.eyebrow` sem uso, tag-link de `CourseResources.astro`, ausência de teste de pertinência de F-08, Q-RN02, e o achado — observado e não explicado — de o Workers Builds não ter disparado para o commit docs-only `db719f4` do 052) |
| v0.1.54 | 2026-09-23 | Desenvolvedor | **Plano 053 DONE** (`876d89c`) — promoção, e com ela a **fase 3 concluída em 12/12**. Três ciclos de revisão: o 1º reprovou com 7 obrigatórios, o 2º só por dois textos, e o 3º (APROVADO, sem obrigatório) corrigiu os dois — a citação `PRD.md:298` **estava certa** no `23a003d` e só se deslocou com o crescimento do §0.1 (por isso a âncora da F-08 passa a ser "§5.4, linha F-08"), e a nota de LF do plano, que continha um byte CR literal, foi reescrita com o instrumento válido (`tr -cd '\r' | wc -c` → 0). Portão local do ciclo 3 verde (263 testes em 16 arquivos + `test:dist` 9; `astro check` 0/0/0). CI `conclusion: success` no run 35930022341 e Workers Builds `success` (versão `24da15df`), os dois sobre `876d89c` |
| v0.1.55 | 2026-09-23 | Desenvolvedor | **Fase 3 fechada formalmente** (`/fechar-fase`). Os 19 planos (036–054) estão DONE, com Evidência. Suíte do fechamento: 265 testes em 16 arquivos, cobertura 100% em statements, branches, funções e linhas (o ramo `href` sem barra de `isActivePath`, único branch descoberto, ganhou 2 testes em `bc17746`, provados por canário); `test:dist` 9/9; `astro check` 0/0/0; `npm audit --audit-level=high` exit 0. Revisão de integração da fase APROVADA sem obrigatório; suas seis observações entraram como dívidas, com "fecha quando", no README da fase. Entrada da fase 3 em `docs/CHANGELOG.md`. Nenhuma ADR nova |
| v0.1.56 | 2026-09-24 | Desenvolvedor | **Redesenho e polimento do site público** (branch `design`, não integrada): cartão contido, cabeçalho de página compacto, títulos na margem esquerda com metadado numa linha acima, sem contagens nem numerais, transições entre páginas (View Transitions), GSAP sob demanda. Emendas: RF-21 (as áreas de atuação passam para a Home; a Sobre ganha a atuação profissional), §7.3 (`perfil.atuacao[]`, criado no redesenho, sem par no grupo `en`), §8.2 (a direção visual aponta para a identidade revisada), §12 fase 3 (nota de que `PillButton` e `Tag` saíram) e fase 4 (item novo para traduzir `atuacao[]`). `docs/identidade-visual.md` §1–§7 reescritas para o site atual. Nenhum item do §12 muda de estado. |
| v0.1.57 | 2026-09-24 | Desenvolvedor | **Branch `design` integrada à `main`** em `00fdef9` (fast-forward de 21 commits sobre `5c38a28`), com CI `qualidade` e Workers Builds verdes (produção na versão `44a1c293`). Além do registrado na v0.1.56, entraram menu sem "Início" (a volta à Home é o nome), seta de volta no lugar da trilha da disciplina, contato da Sobre também na Home, título e régua parados com só o conteúdo rolando, e sublinhado que segue o cursor no menu e no contato. Nenhum item do §12 muda de estado; a fase 4 parte da `main`. |
| v0.1.58 | 2026-09-24 | Desenvolvedor | **Sabatina da fase 4** (`docs/sabatinas/CHANGELOG_sabatina_fase-4-i18n.md`, 12 decisões, das quais a 1 e a 2 foram substituídas pela 3 na própria sessão). A disciplina usa em `/en` o mesmo slug PT (RF-29); marcação de fallback é um aviso por página mais `lang="pt-BR"` no elemento (F-07, RF-28); seletor é um link só para o outro idioma (RF-29); rotas EN são páginas finas sobre uma view por rota (§7.5); tradução de `formacao[]`, `areas[]` e `atuacao[]` vive dentro do item, e `areas[]` vira lista de objetos (§7.3); `atuacao[]` traduz só o `cargo`; 404 em inglês em `/en/404.html`, a provar no Worker (RF-27); sitemap por `@astrojs/sitemap`, robots segue `Disallow` até a Q-05 (RF-30); `en.ts` revisado pelo stakeholder; M-07 provada por teste sobre o `dist/`. Nenhum item do §12 muda de estado; a fase 4 fica sem pendência e pronta para o `/fatiar` |
| v0.1.59 | 2026-09-24 | Desenvolvedor | **Duas questões do fatiamento da fase 4** respondidas e registradas como Decisões 13 e 14 da sabatina (`docs/sabatinas/CHANGELOG_sabatina_fase-4-i18n.md`): texto em PT sem campo em inglês recebe `lang="pt-BR"` sem ligar o aviso, com título e veículo de publicação sem `lang` (RF-28 e F-07 emendados); pares do sitemap pela opção `serialize` da mesma integração se a `i18n` não parear rotas traduzidas (RF-30). **Decisão 15:** Publicações fica sem resumo (título, ano, autores, veículo e links), e o exemplo do RF-28 passa para a linha de pesquisa sem `en.resumo` em `/en/research`. Fase 4 fatiada em 19 planos, 055–073 (`plans/fase-4-internacionalizacao/`) |
| v0.1.60 | 2026-09-25 | Desenvolvedor | **Planos 055 e 056 DONE** (fase 4): `scripts/comparar-dist.mjs` (retrato normalizado do `dist/`, que mantém os `<script>` e compara o script externo pelo conteúdo — emendado duas vezes nas revisões) e `src/lib/routes.ts` + `navItems(locale)`. Executados em modo de teste com `superpowers:executing-plans` e fechados pelo triage e `code-reviewer` da casa; CI e Workers Builds verdes em `48b9a4f` |
...
| M-07 | Rotas `/en` sem string de interface em português | N/A | 0 ocorrências | Inspeção manual de todas as rotas EN + teste sobre o `dist/` que reprova valor do dicionário `pt` no HTML de `/en/**` fora de elemento `lang="pt-BR"` (sabatina fase 4, Decisão 12) | Fase 4 |
...
| RF-28 | MUST | Rotas em inglês sob `/en` com fallback para português por item | Dada uma linha de pesquisa com `en.titulo` e sem `en.resumo`, quando o visitante abre `/en/research`, então vê o título em inglês e o resumo em português, com o elemento marcado `lang="pt-BR"` e um aviso único na página de que parte do conteúdo só existe em português (F-07), e a página nunca fica vazia. Texto em PT **sem** campo em inglês (ex.: título de aula) recebe `lang="pt-BR"` mas não liga o aviso; título e veículo de publicação ficam sem `lang` (sabatina fase 4, Decisões 13 e 15) | ⬜ |
| RF-29 | MUST | Seletor de idioma no cabeçalho: um link só, para o outro idioma ("EN" nas páginas PT, "PT" nas EN), visível também no celular | Dado o visitante em `/ensino`, quando clica em EN, então vai para `/en/teaching` — a mesma página, não a home; em `/ensino/<slug>`, vai para `/en/teaching/<slug>`, com o mesmo slug (sabatina fase 4, Decisões 3 e 5) | ⬜ |
| RF-30 | MUST | SEO básico: `<title>`, meta description, canonical, `hreflang`, Open Graph, favicon, `sitemap.xml`, `robots.txt` | Dado o build de produção, quando se inspeciona o HTML de cada rota, então todos os elementos estão presentes e o sitemap lista todas as rotas públicas de ambos os idiomas, e nenhuma rascunho. Sitemap por `@astrojs/sitemap` com pares `alternate` (pela opção `serialize` a partir do mapa de rotas, se a opção `i18n` não parear os segmentos traduzidos — Decisão 14) e sem as 404; `hreflang` com `x-default` na versão PT (sabatina fase 4, Decisão 10) | ⬜ |
...
| `formacao[]` | lista de objetos | | ✔ (título) | `{ grau, curso, instituicao, ano, en? }` — `en.{grau, curso}` dentro do próprio item (sabatina fase 4, Decisão 7) |
| `atuacao[]` | lista de objetos | | ✔ (cargo) | `{ cargo, instituicao, periodo, en? }` — criado no redesenho (2026-09-23), timeline da Sobre; `en.cargo` dentro do item, a implementar na fase 4 (sabatina fase 4, Decisões 7 e 8) |
| `areas[]` | lista de objetos | | ✔ | `{ nome, en? }` — passa de lista de string a objeto para a tradução viver no item (sabatina fase 4, Decisão 7) |
...
│   │   └── en/            # espelho das rotas em inglês — páginas finas que chamam a mesma view da rota PT (sabatina fase 4, Decisão 6)
...
**Site público.** Direção visual: minimalista e moderno, tipografia forte como elemento principal, movimento discreto com `motion`/GSAP — nunca decorativo a ponto de atrasar a leitura. Referências visuais fornecidas pelo stakeholder em 2026-09-14 (Q-04, `ref/`) e adaptadas ao projeto em **`docs/identidade-visual.md`** — paleta monocromática, Helvetica no texto e Archivo auto-hospedada nos títulos, réguas de 1 px, o site num cartão contido, títulos na margem esquerda e sem contagens. O documento foi revisado em 2026-09-24 para o site depois do redesenho e do polimento, e é o que a fase 4 espelha nas rotas `/en`. Restrições que a identidade deve respeitar, independentemente da referência escolhida: contraste AA, `prefers-reduced-motion` honrado (RF-32), leitura confortável em 360 px, e nenhuma animação bloqueando a exibição do conteúdo.
...
| Integração | Rotas geradas: cada disciplina publicada gera página; nenhuma rascunho aparece no sitemap; toda rota PT tem par EN; nenhum valor do dicionário `pt` no HTML de `/en/**` fora de `lang="pt-BR"` (M-07) | Vitest sobre o `dist/` do build | Fluxos principais |
...
| Fase 4 — Internacionalização | 0/9 | ⬜ Não iniciada |
...
- [x] Layout base, cabeçalho, rodapé e navegação — plano **042** (`db96df3`)
- [x] Identidade visual aplicada — tokens, escala tipográfica e classes de movimento no plano **036** (`fb117b6`); componentes de base (`PageHeader`, `PillButton`, `Tag`, `ExternalLink`) no **043** (`dfdc0a3`) — `PillButton` e `Tag` foram removidos no polimento de 2026-09-24 (branch `design`), e o `PageHeader` virou o cabeçalho compacto. **Conferida rota a rota no 053**, por leitura do `dist/` do triage (2026-09-23 14:38): nas 8 rotas (7 páginas + `404.html`) há exatamente um `<h1>`, a assinatura do `PageHeader` (`titulo-entrada` no `<h1>`, `regua-entrada` na régua — a Home replica a mesma marcação sem importar o componente), uma folha de estilo auto-hospedada em `/_astro/*.css` e zero `fonts.googleapis` — ver a tabela na Evidência do 053
- [x] Home (RF-20) — plano **044** (`4e48ce5`)
- [x] Sobre (RF-21) — plano **045** (`369637a`)
- [x] Pesquisa (RF-22) — plano **046** (`97e006d`)
- [x] Ensino (RF-23) — plano **047** (`4d90d92`)
- [x] Página de disciplina (RF-24) — plano **048** (`5eaf30e`)
- [x] Scripts da disciplina renderizados com destaque de sintaxe e botão de copiar, agrupados por aula (RF-37, F-13) — plano **049** (`1a028ce`)
- [x] Publicações agrupadas por ano (RF-25) — plano **050** (`5b97655`)
- [x] Página 404 (RF-27) — plano **051** (`8c6821c`)
- [x] Responsividade verificada de 360 px a 1440 px (RF-26) — plano **053** (`876d89c`): 8 rotas × 3 larguras, `scrollWidth = clientWidth` nas 24 linhas, medido em 2026-09-23
- [x] Animações com `prefers-reduced-motion` honrado (RF-32); testes da fase passando — plano **053** (`876d89c`): `no-preference` e "sem CSS" observados pelo orquestrador; `reduce` e teclado (RNF-15), verificação manual do stakeholder em 2026-09-23

### Fase 4 — Internacionalização
- [ ] Roteamento i18n configurado (PT na raiz, EN em `/en`)
- [ ] Dicionários `src/i18n/` com todas as strings de interface — o `en.ts` revisado pelo stakeholder antes do DONE (sabatina fase 4, Decisão 11)
- [ ] Nenhuma string de interface hardcoded em componente (§10.4)
- [ ] Utilitário de fallback por campo implementado e testado (RN-06)
- [ ] Rotas EN espelhando as rotas PT, inclusive a 404 em `/en` (RF-27)
...
- [ ] Grupo `en` do `perfil` cobre `atuacao[]` (cargo), no Zod e no Tina, com o teste de paridade — acrescentado em 2026-09-24: o campo nasceu no redesenho sem par em inglês. Pela Decisão 7 da sabatina da fase 4, a tradução de `formacao[]`, `areas[]` e `atuacao[]` vai para dentro de cada item
