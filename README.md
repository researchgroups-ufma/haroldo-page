...
| `npm run test:dist`      | Roda testes de integração sobre o `dist/` gerado (rotas fixas, rascunhos não publicados, um `<h1>` por página, `lang="pt-BR"`, nenhuma fonte de terceiro, JS < 50 KB gzip por rota, zero framework de UI); exige `npm run build:pipeline` ou `npm run build` antes; fica fora de `npm run test` e `test:coverage`. Rodado por um agente de IA, o Vitest esconde a tabela rota → bytes; `npm run test:dist -- --reporter=verbose` a mostra |
...
├── .nvmrc
├── .env.example
├── .github/workflows/    # ci.yml (audit, lint, format:check, test:coverage, build:pipeline, test:dist) e vigia-do-deploy.yml (notifica o ADMIN)
...
`docs/adr/0009-build-de-pipeline-sem-cloud-check.md`). O passo `test:dist` lê o `dist/` recém-gerado e prova que todas as rotas existem, nenhum rascunho foi publicado, a estrutura HTML está íntegra e o JavaScript não viola os limites de tamanho (plano 052, §11). Suíte verde é
